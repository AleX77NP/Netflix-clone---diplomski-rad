...
const Profile = () => {
    ...
    const deactivateProfile = async () => {
        try {
            const res = await fetch(`${baseURL}/${paymentRequests.remove}`,{
                method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'JWT': state.authUser?.token
            ...
            if(res.status === 200) {
                const res2 = await fetch(`${baseURL}/${authRequests.deactivate}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                if(res2.status === 200) {
                    ...
                    toast.dark('Profile deactivated. Be back soon!')
                } else {
                    console.log(res2)
                    toast.dark("An error occurred. Please try again later.")
                }
            } else {
                console.log(res)
                toast.dark("An error occurred. Please try again later.")
    ...
    const deactivation = () => {
        confirmAlert({
            title: 'Confirm to deactivate Your account',
            ...
            <div style={{minHeight: '100vh'}}>
                ...
                <section className={styles.main}>
                    <div className={styles.content}>
                        ...
                        <div className={styles.content_part}>
                            <button onClick={() => deactivation()} className={styles.cancel_btn}>Deactivate Account</button>
